...
if (isset($_POST['formulario_cadastrar_cliente'])) {
    ...
    if ((strlen($cnpjcpf) < 11) and ($cnpjcpf != "")) {
        ...
        $insert = "INSERT INTO `system_day`.`tb_clientes` (`cl_data_cadastro`, `cl_usuario_id`, `cl_razao_social`, `cl_nome_fantasia`, `cl_cpf_cnpj`,
            `cl_inscricao_estadual`, `cl_cep`, `cl_bairro`, `cl_endereco`, `cl_cidade_id`, `cl_estado_id`, `cl_pais`, `cl_telefone`, `cl_email`, `cl_observacao`, `cl_situacao_ativo`)
             VALUES ('$data_lancamento', '$id_usuario_logado', '$rzaosocial', '$nfantasia',
             '$cnpjcpf', '$ie', '$cep', '$bairro', '$endereco', '$cidade', '$estado', 'BRASIL', '$telefone', '$email', '$observacao', 'S')";
        $operacao_inserir = mysqli_query($conecta, $insert);
        if ($operacao_inserir) {
            $retornar["dados"] = array("sucesso" => true, "title" => "Cliente cadastrado com sucesso");
        }
